...
ENV_EOF
              ...
              echo "Creating systemd service file for coleta_aba..."
              cat <<SERVICE_EOF > /etc/systemd/system/coleta_aba.service
[Unit]
Description=STF Coleta Aba Service
...
ExecStart=/usr/bin/python3 $APP_DIR/coleta_aba.py
Restart=always
RestartSec=10
StandardOutput=append:/var/log/coleta_aba.log
StandardError=append:/var/log/coleta_aba.error.log
...
SERVICE_EOF
              ...
              echo "Enabling and starting coleta_aba service..."
              systemctl daemon-reload
              systemctl enable coleta_aba.service
              systemctl start coleta_aba.service
