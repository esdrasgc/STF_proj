...
cat > /etc/systemd/system/coleta-processo.service << EOF
[Unit]
Description=STF Coleta Processo Worker
After=network.target

[Service]
User=root
WorkingDirectory=/app/STF_proj
ExecStart=/usr/bin/python3 coleta_processo.py
Restart=always
...
systemctl enable coleta-processo
systemctl start coleta-processo
  ...
  "logs": {
    "logs_collected": {
      "files": {
        "collect_list": [
          {
            "file_path": "/app/STF_proj/coleta_processo.log",
            "log_group_name": "/stf-proj/coleta-processo",
